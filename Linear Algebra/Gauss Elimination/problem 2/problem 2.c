...
int main(){
	...
	printf("Number of rows are %d \nNumber of columns are %d ",m,n);           // reading augumented matrix from file
	fclose(fp1);
	
       float m2[m][n];
	fp1= fopen("augumented matrix.txt","r");
	while(!feof(fp1))
	{
		for(i=0; i<m; i++)
		{
		...
		{
		for(i=0;i<m;i++)
		{                                                                         	// condition check for zero pivot elements
			if(m2[i][i]==0)
			{
				for(g=i+1; g<n-1; )
			{
		...
		for(p=0; p<n; p++)
		{
			...
			m2[g][p]=temp;
				...
				if(i>j)                                    //  Doing Gauss elimination
			{
					c= ((m2[i][j])/(m2[j][j]));
			...
			printf("\n\nFor Augumented matrix Rows are %d and columns are %d\n\n", m,n);                       // printing matrix after Gauss elimination
					printf("\n\nAugumented matrix after Gauss Elimination:\n\n");                       // printing matrix after Gauss elimination
